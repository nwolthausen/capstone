...
labels <- list(
  variables=list(age_months_b="Age (months)",
                 sex="Sex",
                 F_maternal_education="Caregiver education",
                 maternal_age="Caregiver age"),
  groups=list("", "Acute diarrhea", "")
)

strata <- c(list(Total=ndd), split(ndd, ndd$control_case_group))
...
demo_tbl<- table1(strata, labels, groupspan = c(1, 2, 2),
       render.continuous=my.render.cont, render.categorical=my.render.cat, render = rndr)
demo_tbl

ndd$log_livestock<- as.logical(ndd$F_livestock)

rndr <- function(x, ...) {
    y <- render.default(x, ...)
    if (is.logical(x)) y[1] else y
}



tbl1_assets<- table1(~ F_livestock + F_tv + F_refrigerator + F_electricity + 
                         F_house_floor + F_house_roof + cat_num_liv_hh + WI_scores + WI_quintile
                     |control_case_group, data = ndd, overall = F, extra.col= list(`P-value`=pvalue), na.rm= TRUE)
tbl1_assets

help(table1)

tbl1_WASH<- table1(~ F_toilet_type + shared_toilet + toilet_number_ppl + toilet_location
                   |control_case_group, data = ndd, overall = F, extra.col= list(`P-value`=pvalue), na.rm= TRUE)
tbl1_WASH

tbl1_nut<- table1(~ breastfeed_ever_yn_b + breastfeeding_now + micro_iron + micro_vitamina 
                  + micro_vitamind + micro_zinc + mmd_b + hhds_b
                  |control_case_group, data = ndd, overall = F, extra.col= list(`P-value`=pvalue), na.rm= TRUE)
tbl1_nut
